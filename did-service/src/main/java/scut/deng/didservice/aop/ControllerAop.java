                     ...
                     .map(o -> {
                       if (o instanceof MultipartFile) {
                         return String.format(
                             "{file}, fileName:%s, contentType:%s, fileSize:%d",
                             ((MultipartFile)o).getOriginalFilename(),
                             ((MultipartFile)o).getContentType(),
                             ((MultipartFile)o).getSize());
    ...
    for (Method method : methods) {
      if (method.getName().equals(methodName)) {
        ...
        if (classes.length == args.length) {
          description.append(
              method.getAnnotation(ControllerLogs.class).description());
          break;
        }
      }
    }
