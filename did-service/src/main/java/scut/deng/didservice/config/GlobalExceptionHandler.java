...
public class GlobalExceptionHandler {
  ...
  handleMethodArgumentNotValidException(HttpServletRequest request,
    ...
    try {
      ...
      fieldErrorList.forEach(
          fieldError -> stringJoiner.add(fieldError.getDefaultMessage()));
      log.warn("request API:{} validation failed {}", request.getRequestURI(),
               stringJoiner);
  ...
  handleConstraintViolationException(HttpServletRequest request,
    ...
    Optional<ConstraintViolation<?>> optionalConstraints =
        constraints.stream().findFirst();
    System.out.println("1111111111111111111111111111111111111111111111");
    return optionalConstraints
        .map(constraintViolation
             -> new BaseResponse(ErrorCode.REQ_ERROR,
  ...
  public BaseResponse handleDefaultException(HttpServletRequest request,
                                             Exception ex) {
    log.error("System Exception! request API:{},Exception:{}",
  ...
  public BaseResponse handleMyException(HttpServletRequest request,
                                        Exception ex) {
    ex.printStackTrace();
